...
**Purpose**: This baseline consolidates common security themes across all compliance frameworks (HIPAA, PCI-DSS, SOC 2). Framework-specific compliance extensions reference these baseline rules and add only delta requirements.
...
**Opt-in prompt**: See `security-baseline.opt-in.md` (loaded separately for context efficiency)
...
## Rule SECURITY-16: Change Management (SOC 2 & PCI-DSS)

**Rule**: Changes to systems MUST be controlled:
- **Change documentation**: All changes documented and approved
- **Testing**: Changes tested before production deployment
- **Segregation**: Developers cannot deploy own code to production
- **Emergency changes**: Documented process for emergency changes
- **Rollback**: Ability to revert changes if issues arise

**Verification**:
- Change management process is documented
- Pull request/code review is required
- Separate approval required for production deployment
- Emergency change process exists
- Rollback procedures are documented

**Compliance Context**: Required by PCI-DSS Requirement 6.4 and SOC 2 CC8.1
...
**For Compliance Extensions**: HIPAA, PCI-DSS, and SOC 2 extensions reference these baseline rules. Compliance-specific rules add only delta requirements beyond this baseline.
