...
# PRE-GENERATION PRINCIPLES

## Simplicity First

Before writing any code, apply these principles:

**IMPORTANT**: Simplicity means the simplest *correct* solution. It does NOT mean:
- Taking shortcuts that create technical debt
- Doing workarounds instead of proper fixes
- Skipping necessary error handling or validation
- Ignoring edge cases that matter
...
1. **Prefer the simplest solution** that meets requirements
   ...
   - BUT: Implement what IS requested properly and completely

2. **Impact minimal code** - only touch what's necessary
   - Before modifying, identify the minimum files/lines needed
   - Avoid touching code outside the immediate scope
   - After generation, verify no unrelated files were modified
   - BUT: Touch all the code that DOES need to change for correctness

3. **Prefer editing existing files** over creating new ones
   - Reuse existing utilities, helpers, and patterns
   - Extend existing classes/functions when appropriate
   - Only create new files when existing ones cannot serve the purpose
   - BUT: Create new files when that's the proper architectural choice

4. **No over-engineering**
   - Three similar lines of code is better than a premature abstraction
   - Don't add error handling for scenarios that can't happen
   - Don't create configuration for things that won't change
   - BUT: Add proper error handling for scenarios that CAN happen

## Elegance Check (Full Track Only)

**For Full Track units**, before proceeding to Part 2 (Generation), pause and ask:

1. **"Is this the most elegant approach given what I know?"**
   - Review the plan holistically
   - Consider if there's a cleaner way to achieve the same result
   - Check if existing codebase patterns are being followed

2. **"Would a staff engineer approve this approach?"**
...
3. **If the answer is "no" to either question**:
   - Revise the plan before executing
   - Document why the original approach was rejected
   - Get user approval on revised approach

**For Lightweight and Hotfix Tracks**: Skip the elegance check unless the change feels hacky. Fix first, refactor later if needed.

## Minimal Impact Verification

After completing Code Generation Part 2, verify:

- [ ] No files were modified outside the planned scope
- [ ] No new files were created that duplicate existing functionality
- [ ] No unrelated code was touched "while we're in there"
- [ ] Changes are the minimum necessary to meet requirements

If any verification fails, revert the unintended changes before presenting completion.
...
- [ ] Save complete plan as `aidlc-docs/{domain}/{unit}/construction/plans/{unit-name}-code-generation-plan.md`
...
- [ ] Read the complete plan from `aidlc-docs/{domain}/{unit}/construction/plans/{unit-name}-code-generation-plan.md`
...
- [ ] Write to correct locations:
  ...
  - **Documentation**: `aidlc-docs/{domain}/{unit}/construction/code/` (markdown only)
...
> - **Documentation**: `aidlc-docs/{domain}/{unit}/construction/code/`
...
### Accessibility Standards (Lightweight)

When generating UI code, apply these baseline accessibility checks:
...
- Form inputs: Associate with `<label>` element or use `aria-label`
- Images: Include `alt` text (empty string `alt=""` for decorative)

**Keyboard Navigation**:
- All interactive elements must be focusable via Tab
- No positive `tabindex` values (use only `0` or `-1`)
- Modals must trap focus and dismiss with Escape

**Form Handling**:
- Link error messages to inputs with `aria-describedby`
- Mark required fields with `required` attribute

> **Note**: For comprehensive UI/UX enforcement with blocking findings, enable the UI Baseline extension in TECHNICAL_GUIDELINES.md. See `extensions/ui/baseline/ui-baseline.md` for the full rule set. Additionally, if DESIGN_GUIDELINES.md is configured, UIUX-11 (Design Token Compliance) and UIUX-12 (Interaction Pattern Compliance) will be enforced to ensure adherence to project-specific visual/behavioral standards.
